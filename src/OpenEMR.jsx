...
function MerckManual() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setError(err.message || t('common:errors.searchFailed'));
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      <Header
        title={t('functions:openemr.title')}
        subtitle={t('functions:openemr.subtitle')}
        showBackButton={true}
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-[0_4px_12px_rgba(15,23,42,0.1)] p-6 mb-6 border border-[rgba(15,23,42,0.1)]">
          <h2 className="text-2xl font-bold text-[#1e293b] mb-4">{t('functions:openemr.searchTitle')}</h2>
          <p className="text-[#475569] mb-4">
            {t('functions:openemr.searchDescription')}
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <input
                ...
                placeholder={t('functions:openemr.searchPlaceholder')}
            ...
            {loading && (
              <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center z-10">
                <div className="text-center">
                  ...
                  <p className="text-[#475569]">{t('functions:openemr.loadingContent')}</p>
                  <p className="text-sm text-[#94a3b8] mt-2">{t('functions:openemr.loadingTime')}</p>
                </div>
              </div>
            )}
            <iframe
        ...
        {!hasSearched && (
          <div className="bg-white rounded-xl shadow-[0_4px_12px_rgba(15,23,42,0.1)] p-12 text-center border border-[rgba(15,23,42,0.1)]">
            ...
            <h3 className="text-2xl font-bold text-[#1e293b] mb-4">{t('functions:openemr.professionalTitle')}</h3>
            <p className="text-[#475569] max-w-2xl mx-auto mb-6">
              {t('functions:openemr.professionalDescription')}
            </p>
            <div className="flex justify-center gap-4 text-sm text-[#64748b]">
              <div className="flex items-center">
                ...
                {t('functions:openemr.featurePeerReviewed')}
              </div>
              <div className="flex items-center">
                <svg className="w-5 h-5 mr-2 text-[#3b82f6]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
                {t('functions:openemr.featureProfessional')}
              </div>
              <div className="flex items-center">
                <svg className="w-5 h-5 mr-2 text-[#3b82f6]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                {t('functions:openemr.featureFullText')}
              </div>
            </div>
          </div>
        )}
