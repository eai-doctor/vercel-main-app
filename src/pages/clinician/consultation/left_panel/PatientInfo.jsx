 ...
 const Field = ({ label, viewValue, editNode, emptyText }) => (
  ...
  <div className="p-3">
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-0.5">
      ...
      />
      <Field
        label={t('clinic:consultation.language', 'Language')}
        viewValue={patient_identification?.language || ''}
        editNode={
          <input className={inputCls}
    ...
    {/* Expand toggle */}
    <button
      onClick={() => setShowMoreDemographics(v => !v)}
      className="mt-3 ml-1 hidden items-center gap-1.5 text-xs font-medium text-[#2C3B8D] hover:text-[#233070] transition-colors"
    >
      <span className={`transition-transform duration-200 text-[10px] ${showMoreDemographics ? 'rotate-180' : ''}`}>▼</span>
      {showMoreDemographics
        ? t('clinic:consultation.lessDemographics', 'Less demographic details')
        : t('clinic:consultation.moreDemographics', 'More demographic details')}
    </button>

    {/* Extended fields */}
    {showMoreDemographics && (
      <div className="mt-3 pt-3 border-t border-dashed border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-0.5">
        {[
          { label: t('clinic:consultation.birthplace', 'Birthplace'), key: 'birthplace', placeholder: t('clinic:consultation.cityCountry', 'City, Country') },
          { label: t('clinic:consultation.address', 'Address'),       key: 'address',    placeholder: t('clinic:consultation.streetCityCountry', 'Street, City, Country') },
          { label: t('clinic:consultation.phone', 'Phone'),           key: 'phone',      placeholder: t('clinic:consultation.contactPhoneNumber', 'Contact phone number') },
        ].map(({ label, key, placeholder }) => (
          <Field key={key}
            label={label}
            viewValue={patient_identification?.[key]}
            editNode={
              <input className={inputCls}
                value={editedPatientInfo?.[key] || ''}
                onChange={e => setEditedPatientInfo({ ...editedPatientInfo, [key]: e.target.value })}
        ...
        <Field
          label={t('clinic:consultation.maritalStatus', 'Marital status')}
          viewValue={patient_identification?.marital_status}
          editNode={
            <select className={inputCls}
              value={editedPatientInfo?.marital_status || ''}
              onChange={e => setEditedPatientInfo({ ...editedPatientInfo, marital_status: e.target.value })}
            >
              <option value="">{t('clinic:consultation.selectStatus', 'Select status')}</option>
              {['Single','Married','Separated','Divorced','Widowed'].map(s => (
                <option key={s} value={s}>{t(`clinic:consultation.${s.toLowerCase()}`, s)}</option>
              ))}
            </select>
          }
        />
