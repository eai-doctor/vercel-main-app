import { acceptsRole, LOGIN_PATH } from "@/app/portal";
    ...
    const initializeAuth = async () => {
      try {
        ...
        if (isMounted && userRes.data?.user) {
          if (!acceptsRole(userRes.data.user.role)) throw new Error("Wrong portal role");
...
const login = useCallback(async (email, password) => {
  try {
    ...
    if (!acceptsRole(userData?.role)) {
      clearStoredToken();
      const error = new Error("Use the correct portal for this account.");
    ...
    return { ...userData, access_token: tokenData };
    ...
    const code = err.code === "WRONG_PORTAL" ? err.code : err.response?.data?.code;
    const enrichedError = new Error(err.response?.data?.error ?? err.response?.data?.detail ?? err.message ?? "unknown");
