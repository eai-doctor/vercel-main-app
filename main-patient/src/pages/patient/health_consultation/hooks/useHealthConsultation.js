...
export const useHealthConsultation = (user, accessToken, isAuthenticated, isPatient, loading, openLogin) => {
  ...
  const stopWebSpeechRecognition = useCallback(() => {
    try { speechRecognitionRef.current?.stop(); } catch { /* It may already be stopped. */ }
  ...
  const startWebSpeechRecognition = useCallback(() => {
    ...
    try {
      ...
      r.onend = () => { if (consulting && streamRef.current?.active) { try { r.start(); } catch { /* Recognition may already be active. */ } } };
      r.start();
      speechRecognitionRef.current = r;
    } catch { /* Web Speech is optional and unavailable in some browsers. */ }
  }, [consulting]);
