function getBase(path, envVar, localFallback) {
  ...
  return (import.meta.env.DEV ? localFallback : path).replace(/\/$/, "");
}

const config = {
  backendUrl: getBase("/api/backend", import.meta.env.VITE_BACK_URL, "/api/backend"),
  authServiceUrl: getBase("/api/auth", import.meta.env.VITE_AUTH_URL, "/api/auth"),
  ...
  nutritionConsultationUrl : getBase("http://localhost:4200", import.meta.env.VITE_NUTRITION_CONSULT, "http://localhost:5174")
};
