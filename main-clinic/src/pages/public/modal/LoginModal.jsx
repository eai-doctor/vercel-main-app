...
const isPasswordValid = Object.values(passwordRules).every(Boolean);
  ...
  const safeError = (err) => {
    if (err?.code === "WRONG_PORTAL") return "This account belongs to the other portal. Please switch portals to sign in.";
    if (err?.status === 401) return t("common:errors.invalidCredentials");
    if (err?.status === 429) return "Too many login attempts. Please wait and try again.";
    if (err?.status >= 500) return "The login service is temporarily unavailable. Please try again later.";
    const code = err?.code;
    if (!code) return err?.message && err.message !== "unknown" ? err.message : t("common:errors.generic");
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-[0_20px_60px_rgba(15,23,42,0.15)] max-w-md w-full">
        ...
        <div className="bg-[#2C3B8D] p-5 rounded-t-2xl flex justify-between items-center">
          <div>
            <h2 className="text-white text-xl font-bold">{defaultRole === "clinician" ? "Clinician Portal" : t("patient:portal.title")}</h2>
