...
function TranscribeDictate() {
  ...
  const wsStart = async () => {
    ...
    recognition.onerror = () => {};
    recognition.onend = () => {
      if (wsStreamRef.current?.active) { try { recognition.start(); } catch { /* Recognition may already be active. */ } }
    };
    try {
  ...
  const wsStop = () => {
    ...
    try { wsSpeechRef.current?.stop(); } catch { /* It may already be stopped. */ }
